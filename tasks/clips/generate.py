    ...
    try:
        update_job_status(
            ...
            result_data=build_progress_result_data(
                ...
                extra_result_data={"clip_id": clip_id},
            ),
        )
...
def _release_pending_clip_reservation(
    *,
    ...
    try:
        ...
        logger.warning(
            "[%s] Failed to release %s clip reservation %s: %s",
            job_id,
            charge_source,
            reservation_id,
            exc,
...
def _load_video_source_row(video_id: str) -> dict:
    response = (
        ...
        .select("raw_video_path,raw_video_storage_path,url")
        .eq("id", video_id)
        .limit(1)
        .execute()
    )
...
def generate_clip_task(job_data: GenerateClipJob):
    ...
    try:
            ...
            if charge_source == "team_wallet" and workspace_team_id:
                ...
                if not team_wallet_already_charged:
                    reservation_id = reserve_team_credits(
                        ...
                        team_id=workspace_team_id,
                        amount=generation_credits,
                        reason=f"Clip generation reservation ({job_id})",
        ...
        update_clip_job_progress(
            ...
            progress=5,
            stage="loading_clip",
            detail_key="loading_clip_settings",
        )
        ...
        update_clip_job_progress(
            ...
            detail_key="checking_source_video",
        )
        ...
        def _emit_downloading_source():
            update_clip_job_progress(
                job_id=job_id,
                progress=30,
                stage="downloading_source_video",
        ...
        # Finalize clip record first, then charge credits.
        # This ordering ensures users are never charged for a clip that
        # failed to persist its completion status in the database.
        update_clip_job_progress(
            job_id=job_id,
            progress=90,
            stage="finalizing",
        ...
        if generation_credits > 0 and not team_wallet_already_charged:
            if not reservation_id:
                raise RuntimeError("Missing clip-generation reservation before capture")
            if charge_source == "team_wallet" and workspace_team_id:
        ...
        if _has_retries_remaining():
            logger.warning(
                "[%s] Retries remain but auto-retry is disabled for clip generation failures; "
